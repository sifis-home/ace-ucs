...
public class ObjectSecurityLayer extends AbstractLayer {
	...
	private static final byte[] MARKED = new byte[] { (byte) 0xAA }; 
	 ...
	 */
	public static Response prepareSend(Response message, OSCoreCtx ctx, final boolean newPartialIV, byte[] recipientId) throws OSException {
		return ResponseEncryptor.encrypt(message, ctx, newPartialIV, recipientId);
	}
	...
	public void sendRequest(Exchange exchange, Request request) {
		...
		if (shouldProtectRequest(request)) {
			try {
				...
				//Rikard: Changed to set ID to a certain byte. For outgoing request this value is not used again anyways. 
				exchange.setCryptographicContextID(MARKED); //System.out.println("1");
	...
	public void sendResponse(Exchange exchange, Response response) {
		if (shouldProtectResponse(exchange)) {
			try {
				...
				byte[] rid = exchange.getCryptographicContextID();
				OSCoreCtx ctx = db.getContext(rid); System.out.println(Utils.toHexString(exchange.getCryptographicContextID())); //TODO?
				response = prepareSend(response, ctx, responseWithPartialIV, rid);
	...
	public void receiveRequest(Exchange exchange, Request request) {
		if (isProtected(request)) {
			...
			exchange.setCryptographicContextID(rid); //System.out.println(Utils.toHexString(rid)); //TODO?
		}
		super.receiveRequest(exchange, request);
	}

	@Override
	//This method always accepts unprotected responses
	//This is needed for reception of error messages
	...
	private static boolean responseShouldBeProtected(Exchange exchange, Response response) throws OSException {
		...
		if (exchange.getCryptographicContextID() == null) {
			if (response.getOptions().hasObserve() && request.getOptions().hasObserve()) {
				System.err.println("ERROR: No support for Observe!");
			}
		}
		return exchange.getCryptographicContextID() != null;
	}
