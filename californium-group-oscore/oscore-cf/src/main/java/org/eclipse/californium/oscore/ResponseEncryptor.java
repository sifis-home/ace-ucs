...
public class ResponseEncryptor extends Encryptor {
	 ...
	 */
	public static Response encrypt(Response response, OSCoreCtx ctx, final boolean newPartialIV, byte[] recipientId) throws OSException {
		if (ctx == null) {
		...
		byte[] aad = serializeAAD(response, ctx, false, recipientId); //Rikard: Changed this. Also made sure to set newPartialIV to false.
		//byte[] aad = serializeAAD(response, ctx, newPartialIV, null);
		Encrypt0Message enc = prepareCOSEStructure(confidential, aad);
		byte[] cipherText = encryptAndEncode(enc, ctx, response, newPartialIV, recipientId);
