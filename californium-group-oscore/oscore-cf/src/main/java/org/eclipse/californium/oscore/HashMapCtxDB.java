...
public class HashMapCtxDB implements OSCoreCtxDB {
	...
	public synchronized OSCoreCtx getContext(byte[] rid) {
		...
		OSCoreCtx result = ridMap.get(new ByteId(rid));
		
		if (rid != null && result != null) {
			return result;
		} else if (rid == null) {
		...
		/* Rikard: If no OSCORE Context was found for that rid,
		 * look through all rids in all Group OSCORE Contexts.
		 * Basically searches through the map of rids for all Group OSCORE Contexts,
		 * and checks if that rid has a key (meaning it exists). If so the context is returned.
		 */
		for(OSCoreCtx aCtx : uriMap.values()) {
			if(aCtx instanceof GroupOSCoreCtx) {
				byte[] key = ((GroupOSCoreCtx)aCtx).getRecipientKey(rid);
				if(key != null) {
					return aCtx;
				}
			}
	...
	public synchronized OSCoreCtx getContextByToken(Token token) {
		//System.out.println("getContextByToken");
		if (token != null) {
	...
	public synchronized OSCoreCtx getContext(String uri) throws OSException {
		//System.out.println("getContext(String uri)");
		if (uri != null) {
	...
	public synchronized void addContext(OSCoreCtx ctx) {
		//Rikard: Do not add to rid map of this is Group OSCORE Context since there is no single rid
