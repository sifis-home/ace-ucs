...
public class GroupOSCOREUtils {
	 ...
	 * @param senderKeyPair the full key of the node in base64
	 * @return the created Group OSCORE context
	 * @throws CoseException on derivation failure
	 * @throws AceException on ACE failure
	 */
	public static GroupCtx groupOSCOREContextDeriver(CBORObject joinResponse, String senderKeyPair)
		...
		try {
			senderFullKey = new org.eclipse.californium.grcose.OneKey(
					CBORObject.DecodeFromBytes(Base64.getDecoder().decode(senderKeyPair)));
		} catch (org.eclipse.californium.grcose.CoseException e) {
			System.err.println("Failed to decode sender key!");
			e.printStackTrace();
		}

		return groupOSCOREContextDeriver(joinResponse, senderFullKey.AsCBOR().EncodeToBytes());
	}
	...
	public static GroupCtx groupOSCOREContextDeriver(CBORObject joinResponse,
		...
		org.eclipse.californium.grcose.OneKey senderFullKey = null;
		try {
			senderFullKey = new org.eclipse.californium.grcose.OneKey(CBORObject.DecodeFromBytes(senderFullKeyBytes));
		} catch (org.eclipse.californium.grcose.CoseException e1) {
			// TODO Auto-generated catch block
