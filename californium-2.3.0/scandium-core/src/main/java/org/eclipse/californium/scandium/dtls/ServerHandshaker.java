...
//			LOGGER.error("Cannot compute digest for server's Finish handshake message", e);
	...
	protected void negotiateCipherSuite(final ClientHello clientHello, final HelloExtensions serverHelloExtensions)
		...
		boolean cipherSuiteSelectorResult = cipherSuiteSelector.select(parameters);
		System.out.println("Debug: Server found suitable cipher suit: " + cipherSuiteSelectorResult);

		// System.out.println(publicKey.toString() + certificateChain +
		// clientAuthenticationRequired
		// + clientAuthenticationWanted + commonCipherSuites +
		// commonServerCertTypes.toString()
		// + commonClientCertTypes + commonGroups.toString() + commonSignatures
		// + format);

		if (cipherSuiteSelectorResult) {
